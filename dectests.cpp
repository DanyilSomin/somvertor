...
bool decTests() {
    ...
    ok &= doTest(testStringDecMathChToInt, "testStringDecMathChToInt");
    ok &= doTest(testStringDecMathDigitToChar,
                 "testStringDecMathDigitToChar");
    ok &= doTest(testStringDecMathAdd, "testStringDecMathAdd");
    ok &= doTest(testStringDecMathTwoToThePowerOf,
           "testStringDecMathTwoToThePowerOf");
    ok &= doTest(testStringDecMathModTwo, "testStringDecMathModTwo");
    ok &= doTest(testStringDecMathDivByUint, "testStringDecMathDivByUint");
    ok &= doTest(testStringDecMathToBoolVector,
                 "testStringDecMathToBoolVector");
    ok &= doTest(testStringDecMathFromBoolVector,
           "testStringDecMathFromBoolVector");
...
bool testStringDecMathAdd() {
    ...
    if (StringDecMath::add("59327", "34755653")
            != "34814980") return false;
...
bool testStringDecMathToBoolVector() {
    if (StringDecMath::toBoolVector("0")
            != std::vector<bool>{ }) return false;
    if (StringDecMath::toBoolVector("1")
            != std::vector<bool>{ 1 }) return false;
    if (StringDecMath::toBoolVector("2")
            != std::vector<bool>{ 1, 0 }) return false;
    if (StringDecMath::toBoolVector("3")
            != std::vector<bool>{ 1, 1 }) return false;
    if (StringDecMath::toBoolVector("4")
            != std::vector<bool>{ 1, 0, 0 }) return false;
    if (StringDecMath::toBoolVector("8")
            != std::vector<bool>{ 1, 0, 0, 0 }) return false;
    if (StringDecMath::toBoolVector("29")
            != std::vector<bool>{ 1, 1, 1, 0, 1 }) return false;
    if (StringDecMath::toBoolVector("255")
            != std::vector<bool>{ 1, 1, 1, 1, 1, 1, 1, 1}) return false;
...
bool testStringDecMathFromBoolVector() {
    ...
    if (StringDecMath::fromBoolVector(
        { 1, 1, 1, 1, 1, 1, 1, 1 }) != "255") return false;
