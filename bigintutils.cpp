...
bool validateDigit(char ch, Digits d)
{
    ...
    case Digits::Dec:
        return ch >= '0' && ch <= '9';
    case Digits::Hex:
        return (ch >= '0' && ch <= '9') || (std::toupper(ch) >= 'A' && std::toupper(ch) <= 'F');
    }
...
const std::string add(const std::string &left, const std::string &right, Digits d)
{
...
const std::string divByUint(const std::string &num, uint64_t divisor, Digits d)
{
    if (divisor == 0)
...
const std::vector<bool> toBoolVector(const std::string &num, Digits d)
{
    ...
    return { std::find(reversed.rbegin(), reversed.rend(), true), reversed.rend() };
}
