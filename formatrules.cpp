...
const std::array<FormatRules::Style, 5> FormatRules::styles{
    FormatRules::Style::Plain,      FormatRules::Style::Space, FormatRules::Style::Prefix,
    FormatRules::Style::Programmer, FormatRules::Style::Cpp,
};
...
{
    bool hasStandardPrefix = s == Style::Prefix || s == Style::Programmer || s == Style::Cpp;

    if (hasStandardPrefix && d == BigInt::Digits::Bin)
        return "0b";
    if (hasStandardPrefix && d == BigInt::Digits::Hex)
        return "0x";
...
{
    bool hasSeparator =
        s == Style::Space || s == Style::Prefix || s == Style::Programmer || s == Style::Cpp;

    if (hasSeparator && d == BigInt::Digits::Bin)
        return 4;
    if (hasSeparator && d == BigInt::Digits::Dec)
        return 3;
    if (hasSeparator && d == BigInt::Digits::Hex)
        return 4;
...
{
    const auto separatorRegex =
        separator(s).isEmpty() ? QStringLiteral("") : QString("%1?").arg(separator(s));

    const auto regexString =
        QString("(%1([%3]+%2)*)?|%1%4")
            .arg(prefix(s, d), separatorRegex, charsetRegex(d), QStringLiteral("0"));
...
{
    if (d == BigInt::Digits::Bin)
        return "0-1";
    if (d == BigInt::Digits::Dec)
        return "0-9";
    if (d == BigInt::Digits::Hex)
        return "0-9a-fA-F";
    return "\\d";
}
